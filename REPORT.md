...
| CACHE_LINE_COUNT | 32 линии                              |
| CACHE_SETS       | 8 наборов                         |
...
Другими словами, в классе `LruCacheLine` есть атрибут `access_order` (list) - он как раз отвечет за корректый порядок данных в кэше. Если было обращение к данным, то они перемещаются в конец. Вытеснение происходит из начала
3. Если данных нет, они добавляются в кэш.
В конец `access_order`
...
6. Он находит элемент с самой старой временной меткой. Первый элемнет `access_order`
...
В реализации поддерживается тип `look-through` `write-back`
#### look-through
Это означает, что при чтении (read), данные извлекаются из кэш памяти, если было попадание в кэш
Иначе придется обратиться к основной памяти

#### write-back
При записи (write), данные изменяются в кэш памяти, если было попадание в кэш
Иначе запрос на изменение данных отправляется в основную память

В данном проекте не подразумевается работа с памятью
или кэш-когерентность (политика, при которой множество кэшей используется в разных процессорах)

Поэтому в тех фрагментах кода, где идет запись или обращение к памяти нет корректной реализации работы с памятью

Загрузки

[lh](RiscVSimulator.py#L148) [lw](RiscVSimulator.py#L156) [lb](RiscVSimulator.py#L164) [lbu](RiscVSimulator.py#L172) [lhu](RiscVSimulator.py#L179)

Сохранение

[sb](RiscVSimulator.py#L198) [sh](RiscVSimulator.py#L206) [sw](RiscVSimulator.py#L188)


Если нам было бы необходимо реализовать полный цикл работы с памятью, то в местах, отмеченных сверху (ссылки на фрагменты кода), необходимо было бы совершать проверку на попадание в кеш, а только после этого, при промахе, обращаться в память
(Обращение в память в коде закомментировано)
...
## Обзор
Класс `RiscVSimulator` представляет собой простой симулятор архитектуры RISC-V 32I и RISC-V 32M. Он предназначен для загрузки и выполнения инструкций, а также для управления регистрами, памятью и счетчиком команд (PC). Симулятор также включает механизмы кэширования LRU (Least Recently Used) для оптимизации извлечения инструкций и памяти.
...
```python
def to_int32(x: int): Переводит число или строку в тип int32
```
...
- Функция обрабатывает специальные инструкции такие как `"ecall"`, `"ebreak"`, `"fence"` явно, возвращая предопределенные двоичные строки.
  
## Команды и их записи в двоичный формат

![Здесь должна быть картинка с кодами команд](command_codes_32i.png)
![Здесь должна быть картинка с кодами команд](command_codes_32m.png)
